...
export const SummarySettings = ({ settings, onUpdateSettings }: SummarySettingsProps) => {
  ...
  // Fix: Ensure summaryEnabled is always a boolean
  const summaryEnabled = Boolean(settings?.summaryProvider && settings.summaryProvider !== 'none');
  const hasOpenAIKey = Boolean(settings?.openaiApiKey && settings.openaiApiKey.trim() !== '');
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-sm text-muted-foreground">
            Generate AI summaries of your recordings using OpenAI
          </p>
        </div>
        <Switch
